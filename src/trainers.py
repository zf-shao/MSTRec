...
from metrics import recall_at_k, ndcg_k, precision_at_k, MRR
...
class Trainer:
    ...
    def get_full_sort_score(self, epoch, answers, pred_list):
        recall, ndcg, precision, mrr = [], [], [], []
        for k in [5, 10, 20]:
            recall.append(recall_at_k(answers, pred_list, k))
            ndcg.append(ndcg_k(answers, pred_list, k))
            precision.append(precision_at_k(answers, pred_list, k))
            mrr.append(MRR(answers, pred_list))
            # print("answers:", answers)
            # print("pred_list:", pred_list)
        post_fix = {
            "Epoch": epoch,
            "Recall@5": '{:.4f}'.format(recall[0]), "NDCG@5": '{:.4f}'.format(ndcg[0]), "Precision@5": '{:.4f}'.format(precision[0]),
            "Recall@10": '{:.4f}'.format(recall[1]), "NDCG@10": '{:.4f}'.format(ndcg[1]), "Precision@10": '{:.4f}'.format(precision[1]),
            "Recall@20": '{:.4f}'.format(recall[2]), "NDCG@20": '{:.4f}'.format(ndcg[2]), "Precision@20": '{:.4f}'.format(precision[2]),
            "MRR": '{:.4f}'.format(mrr[0]),
        }
        self.logger.info(post_fix)

        return [recall[0], ndcg[0], precision[0], recall[1], ndcg[1], precision[1], recall[2], ndcg[2], precision[2], mrr[0]], str(post_fix)
